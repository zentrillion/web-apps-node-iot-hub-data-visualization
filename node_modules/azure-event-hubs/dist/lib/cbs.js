            ...
            try {
                ...
                if (!this._isConnectionOpen()) {
                    ...
                    const params = {
                        ...
                        userAgent: connectionContext_1.ConnectionContext.userAgent
                    };
                    this._context.connection = yield utils_1.defaultLock.acquire(this._context.connectionLock, () => { return rpc.open(params); });
                ...
                if (!this._cbsSenderReceiverLink) {
                    ...
                    this._cbsSenderReceiverLink = yield rpc_1.createRequestResponseLink(this._context.connection, { target: { address: this.endpoint } }, rxOpt);
                    ...
                    debug("[%s] Successfully created the cbs sender '%s' and receiver '%s' links over cbs session.", this._context.connectionId, this._cbsSenderReceiverLink.sender.name, this._cbsSenderReceiverLink.receiver.name);
                }
                else {
                    debug("[%s] CBS session is already present. Reusing the cbs sender '%s' and receiver '%s' links over cbs session.", this._context.connectionId, this._cbsSenderReceiverLink.sender.name, this._cbsSenderReceiverLink.receiver.name);
                }
            }
            catch (err) {
                err = errors_1.translate(err);
                debug("[%s] An error occured while establishing the cbs links: %O", this._context.connectionId, err);
                throw err;
            }
        });
    }
    ...
    negotiateClaim(audience, tokenObject) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const response = yield rpc_1.sendRequest(this._context.connection, this._cbsSenderReceiverLink, request);
                return response;
            }
            catch (err) {
